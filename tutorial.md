...
次に、この結果を BigQuery 内で保存する方法を学びます。
...
## Customer voice データを生成AIを用いて分類

Gemini を使ったアンケートデータの分析を行います。

テーブルに保存したアンケートデータを 1 レコードずつ Gemini に渡して、顧客の声を分類するようなクエリを考えましょう。
...
FROM
  ML.GENERATE_TEXT( MODEL next_drag.gemini_model,
    ...
    STRUCT(1000 AS max_output_tokens,
      0.0 AS temperature) )
```
2. エクスプローラーペインで `next_drag` > `customer_voice_category` を選択し、[**プレビュー**] をクリックして顧客の声の分類結果を確認します。
